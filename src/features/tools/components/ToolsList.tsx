...
export function ToolsList() {
  ...
  if (isLoading) {
    return (
      <View style={styles.centered}>
        <ActivityIndicator />
        <Text style={styles.helperText}>Загружаю инструменты…</Text>
      </View>
    );
  }
  ...
  if (isError) {
    ...
    return (
      <View style={styles.centered}>
        <Text style={styles.errorText}>Ошибка при загрузке инструментов</Text>
  ...
  if (!data || data.length === 0) {
    return (
      <View style={styles.centered}>
        <Text style={styles.helperText}>Инструментов пока нет</Text>
      </View>
    );
  }
  ...
  const renderItem = ({ item }: { item: Tool }) => (
    <TouchableOpacity style={styles.card}>
      <View style={styles.cardHeader}>
        ...
        <Text style={styles.type}>{item.tool_type}</Text>
      </View>
      <Text style={styles.meta}>Категория: {item.tool_category}</Text>
