...
export function WeaponKindsList() {
  ...
  if (isLoading) {
    return (
      <View style={styles.centered}>
        <ActivityIndicator />
        <Text style={styles.helperText}>Загружаю типы оружия…</Text>
      </View>
    );
  }

  if (isError) {
    console.error('Failed to load weapon kinds:', error);
    return (
      <View style={styles.centered}>
        <Text style={styles.errorText}>Ошибка при загрузке типов оружия</Text>
  ...
  if (!data || data.length === 0) {
    return (
      <View style={styles.centered}>
        <Text style={styles.helperText}>Типы оружия пока не добавлены</Text>
      </View>
    );
  }

  const renderItem = ({ item }: { item: WeaponKind }) => (
